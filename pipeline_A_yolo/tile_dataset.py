"""
Prepare the YOLO dataset before training.

This script tiles only the training split and copies the validation and test
splits as they are. The goal is to give YOLO more training samples without
changing evaluation on full images.
"""
...
def tile_image_and_labels(image_path: Path,
    ...
    """
    Cut one image into overlapping tiles and update its polygon labels.

    Args:
        image_path: Path to the full image.
        label_path: Path to the YOLO label file.
        output_images_dir: Folder for tile images.
        output_labels_dir: Folder for tile label files.
        tile_size: Tile width and height in pixels.
        overlap: Fraction of overlap between nearby tiles.

    Returns:
        Number of tiles created from this image.
    """
...
def tile_dataset(input_dir: str,
    ...
    """
    Build the final dataset used by `train.py`.

    Args:
        input_dir: Roboflow export folder.
        output_dir: Folder where the prepared dataset will be saved.
        tile_size: Tile size in pixels.
        overlap: Overlap between nearby tiles.
    """
    ...
    def resolve_split_folder(path_value: str, fallback_key: str) -> str:
        """
        Get the split folder name from a path in `data.yaml`.
        """
